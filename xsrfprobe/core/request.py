...
headers = HEADER_VALUES.copy()  # set the headers
...
def Post(url, action, data):
    ...
    try:
        ...
        response = requests.post(main_url, headers=headers,
            data=data, timeout=TIMEOUT_VALUE, verify=VERIFY_CERT)
        if DISPLAY_HEADERS:
...
def Get(url, headers=headers):
    ...
    try:
        ...
        req = requests.get(url, headers=headers,
            timeout=TIMEOUT_VALUE, stream=False, verify=VERIFY_CERT)
